...
-(void) attachNow: (CDVInvokedUrlCommand*)command
{
    ...
    if (dbFileName1 == NULL) {
        // Should not happen:
        NSLog(@"No db name specified for attach");
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"You must specify database path"];
    } else if (dbFileName2 == NULL) {
        // Should not happen:
        NSLog(@"No db name specified to attach to");
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"You must specify database path"];
    } else {
        ...
        if (db == NULL) {
            // Should not happen:
            NSLog(@"attach: db name was not open: %@", dbFileName1);
            ...
            if(sqlite3_exec(db, [attachCommand UTF8String], NULL, NULL, NULL) == SQLITE_OK) {
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"Attach OK"];
            } else {
                // XXX TBD report error??
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Attach failed"];
            }
        }
    }
